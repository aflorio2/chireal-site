---
name: Marta Memon
image: images/team/marta-memon.jpg
role: phd
gender: f
affiliation: Bielefeld University
translate: false
aliases:
  - M. Memon
  - M Memon
links:
  email: m.memon@uni-bielefeld.de
---

## Research interests

Marta Memon is a PhD student in the $\left\lvert\chi\right\rangle$real (Quantum Information and Real-time evolution in QFT) Emmy Noether research group at Bielefeld University.
Her research focuses on exploring strongly correlated phases of matter through quantum correlations. Her interests include quantum field theory, QFT on curved spacetime, and differential geometry.

## Education

<div class="cv-entry">
<span class="cv-date">2023-2025</span>
<strong class="cv-title">M.Sc. in Physics</strong>
<span class="cv-institution">Milan University, Italy</span>
</div>

<div class="cv-entry">
<span class="cv-date">2024</span>
<strong class="cv-title">Erasmus+ program</strong>
<span class="cv-institution">Leipzig University, Germany</span>
</div>

<div class="cv-entry">
<span class="cv-date">2019-2022</span>
<strong class="cv-title">B.Sc. in Physics</strong>
<span class="cv-institution">Milan University, Italy</span>
</div>

## Research Experience

<div class="cv-entry">

<div class="cv-date"> </div>

<div class="cv-title">
Measurement uncertainties in semiclassical gravity — <i>Master thesis</i>
</div>

<ul class="cv-details">
  <li>Advisor: Rainer Verch, Leipzig University; Markus Fröb, Leipzig University</li>
</ul>

</div>


<div class="cv-entry">

<div class="cv-date"> </div>

<div class="cv-title">
Interacting fields in the perturbative approach to the massless Sine–Gordon model — <i>Practical course</i>
</div>

<ul class="cv-details">
  <li>Advisor: Daniela Cadamuro, Leipzig University</li>
</ul>

</div>


<div class="cv-entry">

<div class="cv-date"> </div>

<div class="cv-title">
Multiplanarizable Quivers — <i>Bachelor thesis</i>
</div>

<ul class="cv-details">
  <li>Advisor: Antonio Amariti, Milan University</li>
</ul>

</div>
